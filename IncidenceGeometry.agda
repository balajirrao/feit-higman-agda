module IncidenceGeometry (O : Set) (_#_ : O → O → Set) where
  ...
  data chain where
    chain-zero : ∀ {e f} (e#f : e # f) → chain e f 
    chain-suc : ∀ {e₀ e₁ e₂} (c : chain e₀ e₁) (e₁#e₂ : e₁ # e₂) → chain e₀ e₂
  ...
  reverse (chain-suc {e₀} {e₁} {e₂} c e₁#e₂) =
                     join (chain-zero (#-sym e₁#e₂)) (reverse c)
  
  infix 4 _≺_ _≻_

  data _≺_ : ∀ {e f : O} (x y : O) → {x#y : x # y} {c : chain e f} → Set where
    ≺-zero : ∀ {e f} → (e#f : e # f) → (e ≺ f) {e#f} {chain-zero e#f}
    ≺-suc : ∀ {e₀ e₁ e₂} (c : chain e₀ e₁) (e₁#e₂ : e₁ # e₂) →
                         (e₁ ≺ e₂) {e₁#e₂} {chain-suc c e₁#e₂}
  
  _≻_ : ∀ {e f} (x y : O) {x#y : x # y} {c : chain e f} → Set
  _≻_ x y {x#y} {c} = (y ≺ x) {#-sym x#y} {reverse c}

  next-elem : ∀ {e f y} {c : chain e f} {x : O} {x#y : x # y}
                (x≺y : (x ≺ y) {x#y} {c}) → O
  next-elem {y = y} x≺y = y

  prev-elem : ∀ {e f x} {c : chain e f} {y : O} {x#y : x # y}
                (x≻y : (x ≻ y) {x#y} {c}) → O
  prev-elem {x = x} x≻y = x

  data irred : {e f : O} (c : chain e f) → Set where
    irred-zero : ∀ {e f} → (e≠f : e ≡ f → ⊥) →
                   (e#f : e # f) → irred (chain-zero e#f) 
    irred-suc : ∀ {e₀ e₁ e₂ e₃ e₁#e₂} (c : chain e₀ e₂) (e₂#e₃ : e₂ # e₃)
                  (e₁≺e₂ : (e₁ ≺ e₂) {e₁#e₂} {c}) (¬e₁#e₃ : (e₁ # e₃) → ⊥) →
                  irred (chain-suc c e₂#e₃)
